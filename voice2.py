...
import threading  # To play audio without blocking video
...
tts_engine.setProperty('rate', 190)  # Increase speed (Default ~150)
tts_engine.setProperty('volume', 1.0)  # Ensure maximum volume

# Function to speak in a separate thread (non-blocking)
def speak(text):
...
prev_word = ""  # To avoid repeating the same word in speech
    ...
    while cap.isOpened():
        ...
        if len(sequence) == 30:
            ...
            try:
                ...
                if max(res) > 0.6:  # Higher confidence threshold to avoid mispronunciations
                    sentence = predicted_word

                    # Speak only if it's a new word
                    if sentence != prev_word:
                        speak(sentence)
                        prev_word = sentence  # Update previous word
